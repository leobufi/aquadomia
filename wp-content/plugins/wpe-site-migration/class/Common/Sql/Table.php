...
class Table {
	...
	public function stow( $query_line, $replace = true, $fp = null ) {
		...
		if ( 'savefile' === $state_data['intent'] || in_array( $state_data['stage'], array( Stage::BACKUP, Stage::IMPORT ) ) ) {
	...
	function process_row( $table, $replacer, $row, $structure_info, $fp, $state_data ) {
		...
		if ( ! $skip_row ) {
			...
			foreach ( $row as $key => $value ) {
				...
				if ( Stage::FIND_REPLACE === $state_data['stage'] && in_array( $key, array_keys( $this->primary_keys ) ) ) {
