...
if ( ! class_exists( 'Astra_Woocommerce' ) ) :
	...
	class Astra_Woocommerce {
        ...
        <?php
          if (is_product_category()) {
            echo '<div class="ast-woocommerce-container absolute-container">';
          } else {
            echo '<div class="ast-woocommerce-container">';
          }
        ?>
			<?php
		}
    ...
    public function single_product_category() {
        ...
        if ( empty( $categories ) || is_wp_error( $categories ) ) {
            echo '<span class="single-product-category">Aucune catégorie trouvée</span>';
            return;
        }
    ...
    public function single_product_top_content($product_type = '') {
      ...
      echo '<div class="single-product-top-main-wrapper">';
          ...
          $top_wrapper_structure = array('category', 'title', 'localisation', 'price');

          foreach ( $top_wrapper_structure as $value ) {
            switch ( $value ) {
              case 'title':
                $duree_formation = get_post_meta( $product->get_id(), 'duree_formation', true );
                /**
                 * Add Product Title on single product page for all products.
                 */
                echo '<div class="single-product-title-duration">';
                ...
                if ( ! empty( $duree_formation ) ) {
                  echo '<span class="single-product-duration">' . esc_html( $duree_formation ) . '</span>';
                }
                echo '</div>';
                break;
              case 'price':
                ...
                break;
              case 'localisation':
                $localisation_formation = get_post_meta( $product->get_id(), 'localisation_formation', true );
                $type_formation = get_post_meta( $product->get_id(), 'type_formation', true );
                /**
                 * Add Product Localisation on single product page for all products.
                 */
                echo '<div class="single-product-localisation-type">';
                if ( ! empty( $localisation_formation ) ) {
                    echo '<span class="single-product-localisation">' . esc_html( $localisation_formation ) . '</span>';
                }
                if ( is_array( $type_formation ) ) {
                    echo '<span class="single-product-type">'. esc_html( implode( ', ', $type_formation ) ) . '</span>';
                } else {
                    echo '<span class="single-product-type">'. esc_html( $type_formation ) . '</span>';
                }
                echo '</div>';
                break;
              case 'category':
    ...
    public function single_product_general_infos_block($product_type = '') {

      global $product;

      function replace_br_with_hr($content) {
        $lines = explode("\n", $content);
        $processed_content = '';
      ...
      $left_wrapper_structure = array('pre-requis', 'public-concern', 'blog');

      foreach ( $left_wrapper_structure as $value ) {
        switch ( $value ) {
          case 'pre-requis':
            $pre_requis = get_field('pre_requis', $product->get_id());
            $structured_pre_requis = wpautop( wp_kses_post( $pre_requis ) );
            $structured_pre_requis = replace_br_with_hr($structured_pre_requis);
            if (! empty($pre_requis)) {
              echo '<div class="first-col-block">';
                echo '<h3 class="inset-title">';
                  echo 'Pré-requis';
                echo '</h3>';
                echo '<div class="first-col-content">';
                  echo $structured_pre_requis;
                echo '</div>';
              echo '</div>';
            }
          break;
          case 'public-concern':
            $public = get_post_meta( $product->get_id(), 'public_concern', true );
            $structured_public = wpautop( wp_kses_post( $public ) );
            $structured_public = replace_br_with_hr($structured_public);
            if (! empty($public)) {
              echo '<div class="first-col-block">';
                echo '<h3 class="inset-title">';
                  echo 'Public concerné';
                echo '</h3>';
                echo '<div class="first-col-content">';
                  echo $structured_public;
                echo '</div>';
              echo '</div>';
            }
          break;
          case 'blog':
            $related_article = get_post_meta( $product->get_id(), 'related_article', true );
            $related_article_url = get_permalink($related_article);
            if (! empty($related_article)) {
              echo '<div class="first-col-block">';
                echo '<h3 class="inset-title">';
                  echo '<a href="'. $related_article_url . '">Lien Blog</a>';
                echo '</h3>';
                  echo '<p>Pour en savoir plus sur l’actualité de la formation plongée,
                  vous pouvez aussi consulter notre blog
                  <a href="'. $related_article_url . '">Aquadomia.pro</a>
                  </p>';
              echo '</div>';
            }
          break;
      ...
      echo '</div>';
      echo '<div class="single-product-second-col">';

      $right_wrapper_structure = array('description', 'image');

      foreach ( $right_wrapper_structure as $value ) {
        switch ( $value ) {
          case 'description':
            $description = get_post_field( 'post_content', $product->get_id() );
            $structured_description = wpautop( wp_kses_post( $description ) );
            if ( ! empty( $description ) ) {
              echo '<div class="single-product-description">';
                echo $structured_description;
              echo '</div>';
            }
          break;
          case 'image':
            $image_url = get_the_post_thumbnail_url( $product->get_id() );
            $video_url = get_post_meta( $product->get_id(), 'video', true );
            $play_icon = Astra_Icons::get_icons('play');
            if ( ! empty( $image_url ) ) {
              echo '<div class="single-product-image">';
              echo '<img src="' . esc_url( $image_url ) . '" alt="' . esc_attr( $product->get_name() ) . '" />';
              if (! empty($video_url)) {
                echo '<div class="video-link-block">';
                echo '<span>' . $play_icon . '</span>';
                echo '<a href="' . esc_url( $video_url ) . '" class="video-link" target="_blank">Regarder la vidéo sur Youtube</a>';
                echo '</div>';
              }
              echo '</div>';
            } else {
              ...
              echo '<img src="' . $logo_url . '" alt="Logo Aquadomia" />';
              echo '</div>';
            }
          break;
        }
    ...
    public function single_product_large_attributes_block($product_type = '') {
          ...
          if (!empty($terms)) {
              echo '<div class="main-wrapper large">';
                echo '<h3 class="inset-title">' . wc_attribute_label($attribute->get_name()) . '</h3>';
                  ...
                  $columns = array_chunk($terms, $terms_per_column);
    ...
    public function single_product_short_attributes_block($product_type = '') {
    ...
    function display_calendar_title_if_present() {
        global $post;
      ...
      $this->display_calendar_title_if_present();
		}
