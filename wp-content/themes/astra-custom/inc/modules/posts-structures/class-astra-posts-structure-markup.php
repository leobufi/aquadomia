...
class Astra_Posts_Structure_Markup {
	...
	public function astra_force_render_banner_layout_1() {
        ...
        <div class="cat-infos_container">
          <?php
            if (is_product_category()) {
                $term = get_queried_object();
                $category_image = get_term_meta($term->term_id, 'thumbnail_id', true);
                $image_url = wp_get_attachment_url($category_image);

                if ($image_url) {
                    echo '<div class="category-thumbnail-image">';
                    echo '<img src="' . esc_url($image_url) . '" alt="' . esc_attr($term->name) . '" />';
                    echo '</div>';
                    echo '<div class="category-infos">';
                }
            }
          ?>
          <?php
      ...
      <?php
        if (is_product_category()) {
          ...
          echo '<span class="show-more-btn">+</span>';
          echo '</div>';
          echo '</div>';
        }
      ?>
      <?php
        /* Affichage de l'image bannière */

          $term = get_queried_object();
          $banner_image = get_field('image_banner_woo_categories', $term);

          if ($banner_image) {
            echo '<div class="category-banner-image">';
